...
def zonal_stats_multi(
    ...
    **kwargs
    ...
    # we will require that all the input rasters are the same in terms of shape,
    # resolution, and nodata value - to do otherwise would be nice but much more
    # complicated
    firstRaster = rasterlist[0]
    singleAffine = None
    singleSize = None
    singleNDV = None
    with Raster(firstRaster, affine, nodata, band_num) as rast:
        singleAffine = rast.affine
        singleSize = rast.shape
        singleNDV = rast.nodata

    # we will be going over the features multiple times, once for each raster, +1,
    # so a generator would have to be recreated each time
    features_list = []
    isPreloadedWKT = False
    if geom_attr is not None:
        # Support features where the geometry is encoded on them as a WKT string
        # This is a special case for use within FME: the FME workspace will pass
        # in geometries that are created "on the go" and don't exist on disk,
        # for extraction against a directory full of identical-transform rasters
        features_list = list(vectors)
        isPreloadedWKT = True
    else:
        geom_attr = "geometry"
        features_list = list(read_features(vectors, layer))

    #results = []

    # make one pass through the features to rasterize them all but do not do
    # the overlay / stats calculation in this loop. instead save the rasterized
    # features
    zoneFeatureRasters = {}
    globL = np.inf
    globB = np.inf
    ...
    for i, feat in enumerate(features_list):
        # support wkt pre-encoded
        if isPreloadedWKT:
            try:
                geomWKT = feat[geom_attr]
            except KeyError:
                print "No geom attr found in feature - skipping!"
                continue
        ...
        # Current version of rasterstats doesn't do this anymore, need to work out why?
        geom_bounds = list(geom.bounds)

        # Record the overall bounds of the features as now we are rasterizing
        # once for all raster inputs we need to read the inputs to this overall
        # bounds
        ...
        # This is a key change from earlier version of rasterstats which used
        # gdal rasterizelayer. That in turn requires an OGR layer datasource.
        # Running Python within FME Server (on linux) OGR is broken because
        # FME has its own modified libgdal and libogr which have no drivers
        # so the ogr memory data source cannot be created.
    ...
    # now go through all the rasters
    for rastFileName in rasterlist:
        rastresults = []
        print rastFileName
        with Raster(rastFileName, affine, nodata, band_num) as rast:
            if (rast.affine != singleAffine or
                singleSize != rast.shape or
                singleNDV != rast.nodata):
                print "Raster " + rastFileName + "has differing size, "+\
                    "geotransform, or shape from others - skipping!"
                continue

            # if global_src_extent:
            #      global_src_array = rast.read(bounds = glob_bounds)
            for i, feat in enumerate (features_list):
                fid = None
                try:
                    fid = feat[id_attr]
            ...
            #    print fid
                ...
                if zoneFeatureRasters[fid] is None:
                    ...
                    print "no rasterized version of this feature "
                    continue
                zone_array = zoneFeatureRasters[fid]["zonearray"]
                zone_bounds = zoneFeatureRasters[fid]["bounds"]

                #if not global_src_extent then
                fsrc = rast.read(bounds=zone_bounds)
                ...
                # to output the source feature attributes as well as the stats results,
                # the new rasterstats outputs as geojson features.
                # it's not likely we'd want to do this when running against many rasters
                # as there would have to be one copy of each feature for each raster.
                # Just revert to the more manageable original rasterstats behaviour of
                # outputting a list of attributes for each raster and feature
                if copy_properties:
                    if isPreloadedWKT:
                        for key, val in feat.iteritems():
                            if key == id_attr or key == geom_attr or key == "geometry":
                                continue
                            feature_stats[key] = val
                    else:
                        if 'properties' in feat:
                            for column,value in feat['properties'].iteritems():
                                feature_stats[column] = value

                # removed this as this will not work with wkt input items
                #if geojson_out:
                #    for key, val in feature_stats.items():
            ...
            yield {'rastername' : rastFileName, 'stats' : rastresults}

    ##return results

g=zonal_stats_multi(r'Z:\hsg\Temp\test_zonal_zones\admin_0_2008_Africa_tmp.shp', [r'Z:\hsg\Temp\test_zonal_grids\LST_Night.2011.01.5km.Mean.tif'])
print g.next()
